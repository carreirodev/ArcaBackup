...
| P-28 | **`UEFI:Removable Device` alcança o `ARCABOOT`?** Aberta em 24/08/2026, no fechamento de P-22. As três entradas que o firmware acrescentou **não têm `device` nem `path`** — só `identificador` e `description` —, e `alcanca_o_arcaboot` devolve `false` quando o alvo é `None`. O ARCA as lê como *não levam ao dispositivo*, que é a resposta tranquilizadora. Mas `UEFI:Removable Device` é a classe que boota o primeiro removível, e o `ARCABOOT` é um SSD USB removível. **O modo de falha não é a tela mentir sobre o nome — é ela engolir o aviso**: com aquela entrada em primeiro e a `ARCA` em segundo, a linha sai `dispositivo em 2o de 5 · UEFI:Removable Device vem antes`, correta ao pé da letra, e o parágrafo de perigo não aparece porque ele mora só no ramo em que o dispositivo é o primeiro. Quem lê *"vem antes"* entende que está seguro, e todo reinício boota no dispositivo. É a terceira forma da falha que o ADR-0009 pegou uma vez — *"diria 'o Windows vem antes' e engoliria o aviso"* — e C-6 pegou noutra: aqui não é nome errado nem alvo errado, é a **ausência** de alvo virando segurança. **Não é urgente**: as três estão em 3º, 4º e 5º, atrás do Windows, e quem decide o boot é a primeira. Fecha com um F12 escolhendo aquela linha em vez da entrada `ARCA`, e a pergunta é onde a máquina para. Ver [ADR-0020](../docs/adr/0020-o-bcdedit-enum-firmware-le-a-nvram.md) |
...
| P-28 | **`UEFI:Removable Device` alcança o `ARCABOOT`?** — ver §3.5. Aberta em 24/08/2026, no fechamento de P-22. As entradas que o firmware acrescenta não declaram alvo, e `alcanca_o_arcaboot` lê a ausência de alvo como *não leva ao dispositivo*. Com uma delas em primeiro, o `arca status` acerta o nome de quem vem antes e **engole o aviso de perigo**. Não é urgente — as três estão atrás do Windows —, e fecha com um F12 escolhendo aquela linha |
