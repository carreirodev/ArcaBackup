...
**Nem toda entrada dela diz para onde aponta.** As que o firmware acrescenta trazem só `identificador` e `description` — sem `device` nem `path` —, e o ARCA lê a ausência de alvo como *não leva ao dispositivo*. É P-28, e hoje não morde porque elas estão atrás do Windows.
